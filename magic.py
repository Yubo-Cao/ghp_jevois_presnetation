...
class PyDetectionDNN:
    ...
    def __init__(self):
        ...
        if model == "MobileNetSSD":
            ...
            modelname = (
                pyjevois.share + "/opencv-dnn/detection/MobileNetSSD_deploy.caffemodel"
            )
            configname = (
                pyjevois.share + "/opencv-dnn/detection/MobileNetSSD_deploy.prototxt"
            )
            self.rgb = False
            ...
            classnames = (
                pyjevois.share + "/opencv-dnn/detection/opencv_face_detector.classes"
            )
            modelname = (
                pyjevois.share + "/opencv-dnn/detection/opencv_face_detector.caffemodel"
            )
            configname = (
                pyjevois.share + "/opencv-dnn/detection/opencv_face_detector.prototxt"
            )
            self.scale = 1.0
    ...
    def postprocess(self, frame, outs):
        ...
        def drawPred(classId, conf, left, top, right, bottom):
            ...
            cv.rectangle(
                frame,
                (left, top - labelSize[1] - 2),
                (left + labelSize[0], top + baseLine),
                (255, 255, 255),
                cv.FILLED,
            )
            cv.putText(
                frame, label, (left, top), cv.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0)
            )
        ...
        boxes = []
        if (
            self.net.getLayer(0).outputNameToIndex("im_info") != -1
        ...
        if len(self.outNames) > 1:
            ...
            for cl in unique_classes:
                ...
                nms_indices = cv.dnn.NMSBoxes(
                    box, conf, self.confThreshold, self.nmsThreshold
                )
    ...
    def process(
        ...
        if (
            ...
            self.net.setInput(
                np.array([self.inpHeight, self.inpWidth, 1.6], dtype=np.float32),
                "im_info",
            )
        ...
        cv.putText(
            frame,
            "JeVois Python Object Detection DNN - " + self.model,
            (3, 15),
